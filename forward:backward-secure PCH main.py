...
def Hash(cpabe,pk, h_msg, policy_str):
    ...
    return ctxt, {'g_ch':g_ch, 'b_ch':b_ch, 'h_msg':h_msg, 'r':r}, {'g':g, 'R':zkR, 'rHat':rHat, 'c':c}
...
def Verify(cpabe, pk, ch, zk):
    ...
    b_prime = g ** ch['h_msg'] * ch['g_ch'] ** ch['r']
...
def Adapt(cpabe,pk, sk_tx, ctxt, key, ch, zk):
    g = pk['g']
    h = pk['h']
    group = cpabe.get_group_obj()
...
def main():
    ...
    ctxt, ch, zk = Hash(cpabe,pk, h_msg, policy_str)
    ...
    Adapt(cpabe, pk, sk_tx, ctxt, key, ch, zk)
